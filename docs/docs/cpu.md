...
openJiuXiang是顺序双发射五级流水线结构。流水级包括两个取指单元、译码单元、执行单元、写回单元、InstrBuffer单元、BPU单元、乘除法器单元。缓存包含指令Icache、数据Dcache单元。成功通过龙芯杯官方框架提供的功能测试、性能测试。在50M频率下取得与open-la500 1.5倍加速比。
